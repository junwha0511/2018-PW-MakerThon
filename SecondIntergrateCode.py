...
#초기 선언
fnt = ImageFont.truetype('C:/Users/Junwha-PC/Documents/programming/2018-PW-MakerTon/asdf.ttf', 30)#글꼴 지정
...
for a in arr:
    ...
    image = Image.new("1",((700), 30),"black") #50x너비변수를 가로로, 50을 세로로 하는 이미지 생성
    draw = ImageDraw.Draw(image) #이미지 위에 그린다.
    draw.text((0,0), word, fill = "white", font=fnt) # (0,0)의 위치에서부터 word를 검정색으로 쓴다.
    image.save('C:/Users/Junwha-PC/Documents/programming/2018-PW-MakerTon/img/'+str(count)+'.bmp', "BMP")
    #파싱
    pxArr = ""
    bArr = image.tobytes()

    f = open(('C:/Users/Junwha-PC/Documents/programming/2018-PW-MakerTon/files/'+str(count)+'.h'),'w') #새 파일을 생성한다.
